...
feature_matrix = zeros((2*Number_Of_Channels)*length(girls),6);
Skipped_Subjects = 0;
...
for j=1:Number_Of_Subjects
    ...
    if ~ismember(j,girls)  
        ...
        continue; %Skip subject if its a boys
    end
    
    
    tic
    clearvars -except j feature_matrix botNv topNv Number_Of_Subjects Time_Window L1 Number_Of_Channels Channels girls Skipped_Subjects
    ...
    for jj=1:Number_Of_Channels
        ...
        feature_matrix(jj+(2*Number_Of_Channels*(j-1-Skipped_Subjects)),:) = curr_fv_bot;      
        feature_matrix((jj+Number_Of_Channels)+(2*Number_Of_Channels*(j-1-Skipped_Subjects)),:) = curr_fv_top;
...
attributes=classify_ready(feature_matrix,Number_Of_Channels/2,Number_Of_Channels/2,'girls');
