...
fast = [1 3 4 5 7 10 11 12 13 14 16];
...
feature_matrix = zeros(2*(length(p300_n400_channels)+length(n170_channels))*length(fast),6);     
...
for j=1:Number_Of_Subjects
    ...
    if ~ismember(j,fast)  
        ...
        continue; %Skip subject if its a boy
    end
    
    tic
    clearvars -except j feature_matrix botNv topNv p300_n400_channels n170_channels win_p300 win_n170 Number_Of_Subjects Skipped_Subjects fast 
    %Load subject
...
attributes = classify_ready(feature_matrix,l1,l2,'fast');
